...
func Run(ctx *assets.Context, template *xcore.XTemplate, language *xcore.XLanguage, e interface{}) interface{} {
	...
	// If we are in install mode and L/C set, we change the language of the interface directly into sysparams of Host
	L := ctx.Request.Form.Get("LANGUAGE")
	if L != "" {
		// verify language is authorized

		ctx.Sysparams.Set("language", L)
	}
